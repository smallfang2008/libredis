...
int	writev( int fd, const iovec* p, int len )
{
	int nSend = 0;
	for ( int i=0; i<len; i++ )
	{
		int nT = p->iov_len;
		char* pbuf = (char*)p->iov_base;

		do
		{
			int n = send( fd, pbuf, nT, 0 );
			if ( n<=0 )
				return -1;
			else
			{
...
static int cr_moremem(cr_buffer *buf, int size)
{
  ...
  ptr = (char*) realloc( ( void*)buf->data, total);
  if (ptr == NULL)
    return -1;
...
static int cr_morebulk(cr_multibulk *mb, int size) 
{
  ...
  iptr = (int*)realloc((void*)mb->idxs, total * sizeof(int));
...
static int cr_sendvandreceive2(REDIS rhnd, char recvtype, const struct iovec *vector, int count)
{
  ...
  for (i = 0; i < count; i++) {
    rc = cr_senddata(rhnd->fd, rhnd->timeout, (char*)vector[i].iov_base, vector[i].iov_len);

    if ((unsigned int)rc != vector[i].iov_len) {
      if (rc < 0)
        return CREDIS_ERR_SEND;
      return CREDIS_ERR_TIMEOUT;
    }
